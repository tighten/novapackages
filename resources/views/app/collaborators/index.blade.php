...
<div class="max-w-4xl mx-auto pt-16 px-4 md:px-0">
    ...
    <div class="bg-white p-8 rounded-lg leading-loose text-gray-800 shadow-sm sm:p-12">
        ...
        <blockquote class="border-l-4 border-gray-300 pl-4 text-gray-600 mb-8">You can now claim your ownership over a collaborator. Need to edit it, or claimed the wrong collaborator? For now, just email matt at tighten dot co.</blockquote>
        ...
        @forelse ($unclaimed_collaborators as $collaborator)
            <li><a href="{{ route('collaborators.show', $collaborator) }}" class="text-indigo-600 underline">{{ $collaborator->name }}</a>
                <a href="{{ route('app.collaborators.claims.create', [$collaborator]) }}" class="no-underline bg-gray-600 hover:bg-gray-700 rounded-sm p-1 inline-block mb-1 text-sm text-white">Claim this collaborator</a>
            </li>
        @empty
